...
import { addGeneralPhoto } from "@/app/actions";
...
export default function AddGeneralPhotoDialog({
  generalParamsId,
}: {
        ...
        <form
          ...
          action={async (fd) => {
            ...
            try {
              ...
              const path = `${generalParamsId}/${crypto.randomUUID()}.${ext}`;

              const up = await supabase.storage
                .from("general-photos")
                .upload(path, file, { upsert: false });
              console.log("upload res", up);
              if (up.error) throw new Error(up.error.message);

              const res = await addGeneralPhoto({
